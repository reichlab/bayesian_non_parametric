...
X_train,y_train,X_test,y_test = ts_utils.create_train_test(data__full,1,781)
...
#     y_obs = pm.gp.GP('y_obs', cov_func=f_cov, sigma=s2_n, observed={'X':X, 'Y':ymu})

    Sigma = f_cov(X_train) + tt.eye(len(X_train))*s2_n**2
    y_mu = pm.MvNormal('y_mu', mu=intercept, cov=Sigma, shape=(len(X_train),))
    y_obs = pm.Poisson('y_obs', mu=pm.math.exp(y_mu), observed=y_train)
...
indices = np.random.randint(0, len(temp_trace), nsample)

X1 = X_test
with model2:
    S_xz = f_cov(X1, Z)
    S_zz = f_cov(Z)
    S_inv = matrix_inverse(f_cov(X1) + tt.eye(X1.shape[0])*s2_n**2)

    # Posterior mean
    m_post = tt.dot(tt.dot(S_xz.T, S_inv), ymu)
    # Posterior covariance
    S_post = S_zz - tt.dot(tt.dot(S_xz.T, S_inv), S_xz)

    gp_post = pm.MvNormal.dist(m_post, S_post, shape=Z.shape[0])

    samples = [gp_post.random(point=temp_trace[idx]) for idx in indices]

gp_samples=np.array(samples)
