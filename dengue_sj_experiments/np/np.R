...
train_x_inds <- seq_len(n_train)
train_y_inds <- train_x_inds + 1
test_x_inds <- seq_len(n_test) + n_train
test_y_inds <- test_x_inds + 1
X_train <- f[train_x_inds]
y_train <- f[train_y_inds]
X_test <- f[test_x_inds]
y_test <- f[test_y_inds]
...
ggplot() +
  ...
  geom_point(aes(x = X, y = prediction, color = model),
    data = res %>%
      gather_("model", "prediction", c("true", "np"))) +
