...
sarimaTD_fit <- fit_sarima(
  ...
  ts_frequency = 52,
  transformation = "forecast-box-cox",
...
for(prediction_time_ind in seq_len(nrow(prediction_times))) {
  ...
  write_csv(predictions_cdc_format,
    path = paste0("dengue_sj_experiments/sarima/eval_predictions_forecast_box_cox_transformation_seasonal_differencing/last_obs_",
