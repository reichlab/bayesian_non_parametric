...
with pm.Model() as model:
    ...
    init_1 = np.random.randn(X.shape[1], hidden_size)
    init_1 = init_1[:, np.argsort(init_1.sum(axis=0))]
    init_2 = np.random.randn(hidden_size)
    init_2 = init_2[np.argsort(init_2)]
   # init_4 = np.random.rand(hidden_size,)
    #init_5 = np.random.rand(1)
    # Weights from input to hidden layer
    weights_in_1 = pm.Normal('w_in_1', 0, sd=10, shape=(X.shape[1], hidden_size),
                             testval=init_1)
    #bias_in_1 = pm.Normal('b_in_1', 0, sd=100, shape=(hidden_size,),
                             #testval=init_4)
    # Weights from hidden layer to output
    weights_1_out = pm.Normal('w_1_out', 0, sd=10, shape=(hidden_size,),
                              testval=init_2)
    #bias_in_2 = pm.Normal('b_in_2', 0, sd=100, shape=(49,),
                             #testval=init_5)
    ...
    print (act_out.shape.eval({ \
        weights_in_1:np.ones((1,6)), \
        #bias_in_1:np.ones((6,)),
        weights_1_out: np.ones((6,))
       # bias_in_2:np.ones((49,))
        }))
...
tau_out = trace['tau'][1000:]
#b_1 = trace['b_in_1'][1000:].mean(axis=0)
#b_2 = trace['b_in_2'][1000:].mean(axis=0)
dist = []
for w_elm in range(len(w_1)):
    preds = []
    for elm in data__full[99:149]:
        h = np.dot(w_1[w_elm],np.array(elm))#+b_1
        h = np.tanh(h)
        preds.append(
            scipy.stats.norm.logpdf(x, loc=(np.dot(h,w_2[w_elm])).tolist()[0],scale=tau_out[w_elm]))
    dist.append(preds)


print (np.array(dist).shape)
#print (np.array(SS.inverse_transform(preds)).reshape((-1,)))
##print (data__full[100:150])



#print (mean_squared_error(np.array(SS.inverse_transform(preds)).reshape((-1,)),SS.inverse_transform(data__full[100:150])))

#rint (np.array(preds)-np.array(data__full[49:60]))
