...
test_data = []
test_target = []
for i in range(5,len(data__)):
    train_data.append([data__[i-50:i-1]])
    train_target.append(data__[i])

print (np.array(train_data).reshape((-1,5)))
X_train =   np.array(train_data,dtype=np.float32).reshape((-1,5))
y_train =   np.array(train_target,dtype=np.float32).reshape((-1,1))
X_test = []
y_test = []
for i in range(781,987):
    #x_test2.append(data__full[i-1])
    X_test.append(data__full[i-4:i])
    y_test.append(data__full[i+1])
model=Sequential()
#model.add(SimpleRNN(input_dim  =  49, output_dim = 50, return_sequences = True))
#model.add(TimeDistributed(Dense(output_dim = 1, activation  =  "linear")))
model.add(Dense(512, activation='tanh', input_shape=(49,)))
...
pred = model.predict(np.array(X_test).reshape((-1,5)))
print (pred)

print (mean_squared_error(pred.reshape((-1)),y_test))
...
# num_centr = 50          # Number of "hidden neurons" that is number of centroids
...
#   var = tf.Variable(tf.truncated_normal([num_centr],mean=20,stddev=5,dtype=tf.float32),name='RBF_variance')
...
#         sum_ = 0
#         results = []
#         for test_points in range(780,988):
#           result = sess.run([cost,accuracy,h],feed_dict={x:np.array([data__full[test_points]]).reshape((-1,1)), y_: np.array(data__full[test_points+1]).reshape((1,))})
#           results.append(SS.inverse_transform(result[2][0][0]).tolist())
#           acc = result[1]
#         print (results)
#          # print("Estimated accuracy at iteration %s of %s: %s" % (i,max_iterations, acc))
#         print (mean_squared_error(results,SS.inverse_transform(data__full[781:])))
#         print ("=------------=\n")
#       else:
