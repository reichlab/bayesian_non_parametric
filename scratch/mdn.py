...
hidden_layer =tf.tanh(tf.matmul(x, Wh) + bh)
...
gvs = optimizer.compute_gradients(lossfunc)
...
print ("--------")
def p(x,weights,means,sds):
  sum_ = 0
  for i in range(len(weights)):
    sum_ += weights[0][i]*norm.pdf(x,loc=means[0][i],scale=sds[0][i])
  return sum_

for elm in data__full[49:59]:
  outputs =[]

  weights,means,sds = sess.run([out_pi, out_sigma, out_mu],feed_dict={x: \
    np.array([elm]).reshape((-1,1)), y: y_train})
  for i in range(-20,100):
    outputs.append(p(i,weights,means,sds))
  print (elm,np.argmax(outputs))
