...
model.add(Dense(512, input_shape=(1,)))#,  W_regularizer=l2(weight_decay), U_regularizer=l2(weight_decay),
#                b_regularizer=l2(weight_decay), dropout_W=.1, dropout_U=.1))
model.add(Dropout(.1))
...
model.fit(train_X, train_y, batch_size=1, nb_epoch=250)
...
preds = []
for i in range(1000):
  standard_prob = model.predict(np.array(data__full[49]).reshape((-1,1)), batch_size=1, verbose=0)
  preds.append(standard_prob)

print (np.array(preds).mean())
preds = []
for i in range(1000):
  standard_prob = model.predict(np.array(data__full[1]).reshape((-1,1)), batch_size=1, verbose=0)
  preds.append(standard_prob)
