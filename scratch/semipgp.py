...
inputs = X_train
targets = y_train
m = GPy.models.GPRegression(inputs, targets, k)
m.optimize()
pred_SE, var_SE = m._raw_predict(np.array(data__full[49:60]).reshape((-1,1)), kern=m.kern.Mat52)
print (pred_SE)
print (np.sum((1.0/11)*(pred_SE.reshape((-1,))-np.array(data__full[50:61]))**2))
m.plot()
plt.show()
