...
cond = Input(batch_shape=(batch_size, n_y))
...
z_log_var = Dense(latent_dim,activation='linear')(h)
...
x_decoded_mean = decoder_mean(z_cond)
...
train_X = np.array(train_target).reshape((-1,1))
...
plt.scatter(train_X,train_y,alpha=.1,color='r')


vae.fit([train_X, train_y],train_X,
        shuffle=True,
        nb_epoch=nb_epoch,
...
#d_h = decoder_h(d_in)
d_out = decoder_mean(d_in)
decoder = Model([z_in, y_in], d_out)
# display a 2D plot of the digit classes in the latent space
pred_dist = []
print (data__full[50])
for i in range(1000):
    z_sample = np.random.randn(latent_dim).reshape((1,latent_dim))
    X_gen = decoder.predict([z_sample,np.array([data__full[49]])])
    pred_dist.append(X_gen)
print (np.array(pred_dist).mean())

pred_dist = []
print (data__full[1])
for i in range(1000):
    z_sample = np.random.randn(latent_dim).reshape((1,latent_dim))
    X_gen = decoder.predict([z_sample,np.array([data__full[1]])])
    pred_dist.append(X_gen)
print (np.array(pred_dist).mean())
plt.show()
