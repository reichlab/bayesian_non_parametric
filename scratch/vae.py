...
test_data = []
test_target = []
for i in range(0,len(data__)-1):
    train_data.append(data__[i])
...
train_target = enc.transform(train_target).toarray()
...
decoder_out = Dense(train_target.shape[1], activation='softmax')
...
vae.fit(train_data, train_target, batch_size=m, nb_epoch=n_epoch)


mc_samples = []
num_samples = 1000
for i in range(num_samples):
    max_arg = np.argmax(vae.predict(train_data[13].reshape((1,1))))
    mc_samples.append(max_arg)
    print (train_data[13].reshape((1,1)))
    print (max_arg)

mc_samples = np.array(mc_samples)
print (mc_samples.mean())
print (mc_samples.std())
