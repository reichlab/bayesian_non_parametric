 ...
 */
#ifndef LIBBI_ACTION2_HPP
#define LIBBI_ACTION2_HPP

#include "ActionCoord2.hpp"
...
struct Action2 {
   ...
   * Coordinate type.
   */
  typedef ActionCoord2 coord_type;
  ...
  template <bi::Location L, class CX, class PX, class OX>
  void Action2::simulates(bi::State<ModelSIR,L>& s, const int p, const int ix, const CX& cox, const PX& pax, OX& x)
  
 {
  ...
  const BOOST_AUTO(R__, pax.template fetch<Var5>(s, p, 0));
  /* inlines */
  ...
  template <class T1, bi::Location L, class CX, class PX, class OX>
  void Action2::simulates(const T1 t1, const T1 t2, bi::State<ModelSIR,L>& s, const int p, const int ix, const CX& cox, const PX& pax, OX& x)
