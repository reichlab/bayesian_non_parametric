 ...
 */
#ifndef LIBBI_ACTION23_HPP
#define LIBBI_ACTION23_HPP

#include "ActionCoord23.hpp"
...
struct Action23 {
   ...
   * Coordinate type.
   */
  typedef ActionCoord23 coord_type;
  ...
  template <bi::Location L, class CX, class PX, class OX>
  void Action23::simulates(bi::State<ModelSIR,L>& s, const int p, const int ix, const CX& cox, const PX& pax, OX& x)
  
 {
  ...
  const BOOST_AUTO(t_now__, pax.template fetch<Var0>(s, p, 0));
  
  const BOOST_AUTO(Z__, pax.template fetch<Var8>(s, p, 0));
  /* inlines */
  
  const CX& cox_ = cox;
  x.template fetch<target_type>(s, p, cox_.index()) = ((bi::mod(t_now__, BI_REAL(7)) == BI_REAL(0)) ?BI_REAL(0) : Z__);
}


  
  template <class T1, bi::Location L, class CX, class PX, class OX>
  void Action23::simulates(const T1 t1, const T1 t2, bi::State<ModelSIR,L>& s, const int p, const int ix, const CX& cox, const PX& pax, OX& x)
