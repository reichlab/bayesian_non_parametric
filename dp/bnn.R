...
train_and_predict_bnn <- function(sanJuanDengueData,n_ahead,last_training_point,max_lag_val){
  ...
  write.table(X_test, file = "/Users/gcgibson/bayesian_non_parametric/X_test.csv", append = FALSE, quote = FALSE, sep = ",",
              ...
              col.names = FALSE, qmethod = c("escape", "double"))
  Sys.sleep(30)
  exec_str <- 'python /Users/gcgibson/bayesian_non_parametric/dp/bnn.py'
  bnnForecasts <- system(exec_str,intern=TRUE,wait = TRUE)
  bnnForecasts <- strsplit(bnnForecasts,",")
  bnnForecasts <- as.numeric(unlist(bnnForecasts))
  bnnForecasts
