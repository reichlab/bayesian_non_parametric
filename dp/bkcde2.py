import numpy as np
from autograd import grad
import numpy.matlib as nm
import emcee
import matplotlib.pyplot as plt
import copy
from random import shuffle
import time
from scipy.stats import multivariate_normal
from scipy.stats import norm
import math
import sys
from pyhmc import hmc
from svgd import SVGD



X_train = \
    np.loadtxt(open("/Users/gcgibson/Desktop/bayesian_non_parametric/X_train.csv", "rb"), delimiter=",", skiprows=0)


y_train = \
    np.loadtxt(open("/Users/gcgibson/Desktop/bayesian_non_parametric/y_train.csv", "rb"), delimiter=",", skiprows=0)


X_test = \
    np.loadtxt(open("/Users/gcgibson/Desktop/bayesian_non_parametric/X_test.csv", "rb"), delimiter=",", skiprows=0)




X_train = X_train[:10]
y_train = y_train[:10]



def gaussian_kernel_2d(x,y,h):
    mat = h*np.eye(len(x))
   
    precision_mat = np.linalg.inv(mat)
    tmp = np.dot(np.dot(np.transpose(x-y),precision_mat),(x-y))
    return 1.0/(np.sqrt(np.linalg.det(2*np.pi*mat)))*np.exp(-.5*tmp)






lambda_ = 1

joint_data = np.hstack((X_train,y_train.reshape((-1,1))))




data_x = X_train



def lnprob(theta):
    tmp_theta = np.exp(theta)
    # Rob Hyndman's prior stated here https://robjhyndman.com/papers/mcmckernel.pdf
    ln_prior = np.log(1/(1+lambda_*tmp_theta[0]**2))
    #LOO likelihood averaged over all data points
    likelihood_proposal = 0
    for i1 in range(len(joint_data)):
            loo_likelihood_proposal = 0
            data_mod = copy.copy(joint_data)
            test_x = joint_data[i1]
            data_mod = np.delete(data_mod,(i1),axis=0)
            for i in range(len(data_mod)):
                loo_likelihood_proposal += \
                    1.0/(tmp_theta[0])*gaussian_kernel_2d(np.array([test_x[0]]),data_mod[i][0],tmp_theta[0])* \
                    1.0/(tmp_theta[1])*gaussian_kernel_2d(np.array([test_x[1]]),data_mod[i][1],tmp_theta[1]) * \
                    1.0/(tmp_theta[2])*gaussian_kernel_2d(np.array([test_x[2]]),data_mod[i][2],tmp_theta[2])*\
                    1.0/(tmp_theta[3])*gaussian_kernel_2d(np.array([test_x[3]]),data_mod[i][3],tmp_theta[3]) *\
                    1.0/(tmp_theta[4])*gaussian_kernel_2d(np.array([test_x[4]]),data_mod[i][4],tmp_theta[4])
    
    
            likelihood_proposal += loo_likelihood_proposal
    likelihood_proposal = likelihood_proposal/(1.0*len(joint_data)-1 )
    
    if math.isnan(np.log(likelihood_proposal)) == False:
        return np.log(likelihood_proposal) + ln_prior.sum()
    return -np.inf



def metropolis(init, iters):
    """
        based on http://www.cs.toronto.edu/~asamir/cifar/rpa-tutorial.pdf
        
        can take several minutes to run with large sample sizes.
        """
    
    # set up empty sample holder
    D = len(init)
    samples = np.zeros((D, iters))
    
    # initialize state and log-likelihood
    state = init.copy()
    Lp_state = lnprob(state)
    
    
    accepts = 0
    for i in np.arange(0, iters):
        
        # propose a new state
        prop = np.random.multivariate_normal(state.ravel(), np.eye(5)).reshape(D, 1)
        
        Lp_prop = lnprob(prop)
        
        rand = np.random.rand()
        if np.log(rand) < (Lp_prop - Lp_state):
            accepts += 1
            state = prop.copy()
            Lp_state = Lp_prop
        
        samples[:, i] = state.copy().ravel()
    
    # if i % 1000 == 0: print i
    
    print 'Acceptance ratio', (1.0*accepts)/(1.0*iters)
    return samples







w_0 = np.array([1., 1., 1., 1., 1.])

# actually do the sampling
n = 50000
samples = metropolis(w_0, n)
print (samples.shape)
v = samples[:, 1000:]

print (v.mean(axis=1))
print (v.var(axis=1))










sys.exit()


def get_expected_value(x_test,joint_data,data_x,expected_joint_bandwith):
    expected_value = 0
    joint_density_sum = 0
    marginal_density_sum = 0
    delta_t = .1
    for y_test1 in np.arange(-10,1000,delta_t):
        tmp1 = 0
        tmp2 = 0
        for i in range(len(data_x)):
            what_tmp = x_test + [y_test1]
            tmp1 +=  (delta_t**2)*1.0/(len(joint_data))*gaussian_kernel_2d(what_tmp,joint_data[i],expected_joint_bandwith)
            tmp2  +=  (delta_t)*1.0/len(data_x)*gaussian_kernel_2d(x_test,data_x[i],expected_joint_bandwith[:4])
        expected_value += y_test1*tmp1/tmp2
    
    #expected_value += y_test1*density
    return expected_value

full_season_ahead_predictions = []
for i in range(len(X_test)):
    ex = get_expected_value(X_test[i].tolist(),joint_data,data_x,expected_joint_bandwith)
    full_season_ahead_predictions.append(ex)

print (full_season_ahead_predictions)


