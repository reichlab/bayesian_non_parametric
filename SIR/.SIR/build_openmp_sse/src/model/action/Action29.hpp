 ...
 */
#ifndef LIBBI_ACTION29_HPP
#define LIBBI_ACTION29_HPP

#include "ActionCoord29.hpp"
...
struct Action29 {
   ...
   * Target type.
   */
  typedef Var13 target_type;

  /**
   * Coordinate type.
   */
  typedef ActionCoord29 coord_type;
  ...
  template <bi::Location L, class CX, class PX, class OX>
  void Action29::simulates(bi::State<ModelSIR,L>& s, const int p, const int ix, const CX& cox, const PX& pax, OX& x)
  
 {
  ...
  const BOOST_AUTO(param_aux__13___, pax.template fetch<Var13>(s, p, 0));
  /* inlines */
  
  const CX& cox_ = cox;
  x.template fetch<target_type>(s, p, cox_.index()) = (p_rep__*(BI_REAL(1) - p_rep__));
}


  
  template <class T1, bi::Location L, class CX, class PX, class OX>
  void Action29::simulates(const T1 t1, const T1 t2, bi::State<ModelSIR,L>& s, const int p, const int ix, const CX& cox, const PX& pax, OX& x)
