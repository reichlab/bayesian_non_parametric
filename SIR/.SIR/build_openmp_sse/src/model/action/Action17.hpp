 ...
 */
#ifndef LIBBI_ACTION17_HPP
#define LIBBI_ACTION17_HPP

#include "ActionCoord17.hpp"
...
struct Action17 {
   ...
   * Coordinate type.
   */
  typedef ActionCoord17 coord_type;
  ...
  template <bi::Location L, class CX, class PX, class OX>
  void Action17::simulates(bi::State<ModelSIR,L>& s, const int p, const int ix, const CX& cox, const PX& pax, OX& x)
  
 {
  ...
  const CX& cox_ = cox;
  x.template fetch<target_type>(s, p, cox_.index()) = BI_REAL(1);
}


  
  template <class T1, bi::Location L, class CX, class PX, class OX>
  void Action17::simulates(const T1 t1, const T1 t2, bi::State<ModelSIR,L>& s, const int p, const int ix, const CX& cox, const PX& pax, OX& x)
