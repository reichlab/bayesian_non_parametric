...
for (run in start_run:end_run){
  ...
  mod <- dlmModARMA(ar =coef[1],ma=coef[2],sigma2=exp(coef[3])) + dlmModTrig(s=2,dV=coef[4])
...
exec_str <- 'python /Users/gcgibson/Desktop/bayesian_non_parametric/seasonal_dlm.py '
...
#   geom_line(data=data.frame(x=1:output_length,y=dlmForecasts),aes(x,y,color="UKF"),size=.2)+ 
#   
