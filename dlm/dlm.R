...
sanJuanDengueDataTrainTS <- ts(sanJuanDengueData[seq(1,run)], start = 1, frequency = 52)


n_ahead <- 5


lagged_4 <- cbind(Lag(sanJuanDengueData[seq(1,run-n_ahead)], k=3),Lag(sanJuanDengueData[seq(1,run-n_ahead)], k=2),
                  Lag(sanJuanDengueData[seq(1,run-n_ahead)], k=1),Lag(sanJuanDengueData[seq(1,run-n_ahead)], k=0))

lagged_4 <- matrix(lagged_4,ncol=4)[-1:-4,]

mlpFit <-rbf(lagged_4, sanJuanDengueData[seq(n_ahead,run-n_ahead)], size=50)
...
for (run in start_run:end_run){
  ...
  mod <- dlmModARMA(ar =coef[1],ma=coef[2],sigma2=exp(coef[3])) #+ dlmModTrig(s=52,dV=coef[4])
...
#   #geom_line(data=data.frame(x=1:output_length,y=kcdeForecasts),aes(x,y,color="KCDE"),size=.2)+ 
