...
train_and_predict_ssm <-  function(sanJuanDengueData,n_ahead,last_training_point){
      ...
      stateSpaceCode <- nimbleCode({
        ...
        x[1] ~ dnorm(1, sd = sigPN)
        y[1] ~ dnorm(1 ,sd= sigOE)
        
        for (i in 2:t) {
          x[i] ~ dnorm( x[i - 1] , sd = sigPN)
      ...
      stateSpaceModel <- nimbleModel(stateSpaceCode,
                                     data = list(y=sanJuanDengueData[seq(1,last_training_point)]),
                                     constants = list(t= length(sanJuanDengueData[seq(1,last_training_point)])),
                                     inits = inits,
                                     check = FALSE)
       ## train the model
      ...
      stateSpaceMCMCconf$addSampler(target = c('sigPN','sigOE'),
      ...
      posteriorSamps <- as.matrix(compiledList$stateSpaceMCMC$mvSamples)
   
      inits <- list(
        sigPN = posteriorSamps[,'sigPN'],
        sigOE = posteriorSamps[,'sigOE']
      )
     for (i in seq(last_training_point+1-n_ahead,last_training_point+1+5-n_ahead)){
      stateSpaceModel <- nimbleModel(stateSpaceCode,
                                     data = list(y=sanJuanDengueData[seq(1,i)]),
                                     constants = list(t= length(sanJuanDengueData[seq(1,i)])),
                                     inits = inits,
                                     check = FALSE)
      ...
      theta_mean <- mean(posteriorSamples)
      print (theta_mean)
      for (j in seq(1,n_ahead)){
          theta_mean <- theta_mean 
      }
      print (theta_mean)
      ssmForecasts <-c(ssmForecasts,exp(theta_mean))
      nimble:::clearCompiled(stateSpaceModel) 
    }
ssmForecasts
}

ssmF <- train_and_predict_ssm(sanJuanDengueData,n_ahead,last_training_point)
print(ssmF)
