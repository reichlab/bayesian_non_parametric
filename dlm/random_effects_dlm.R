...
model_file = '/Users/gcgibson/Desktop/bayesian_non_parametric/dlm/re.bug' # BUGS model filename
...
data = list(t_max=t_max, y = sanJuanDengueData[seq(1,780)],prec_x_init=prec_x_init,
            prec_x=prec_x, 
            mean_x_init=1)
...
param_names = c('alpha') # name of the variables updated with MCMC (others are updated with SMC)
latent_names = c('x') # name of the variables updated with SMC and that need to be monitored


obj_pmmh = biips_pmmh_init(model, param_names, inits=list(alpha=1),
