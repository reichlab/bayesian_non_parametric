...
train_and_predict_dlm <- function(sanJuanDengueData,n_ahead,last_training_point){
    ...
    fit1 <- dlmMLE(y=sanJuanDengueDataTrainTS,parm=c(1,1,1),build=dengue_dlm,hessian=T)
    coef <- fit1$par
    
    mod <- dlmModARMA(ar =coef[1],ma=coef[2],sigma2=exp(coef[3]))
    for (i in seq(last_training_point,last_training_point+52)){
