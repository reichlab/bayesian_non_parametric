...
def transition_function(state, noise):
    s = -b*state[0]*state[1] 
    i = b*state[0]*state[1] - k*state[1]
    return np.array([s,i])

def observation_function(state, noise):

    return state[1] + noise

state_size = 2
transition_covariance = np.eye(state_size)
observation_covariance = .001
initial_state_mean = np.array([20000,1])
initial_state_covariance = np.eye(state_size)  * 0.001
...
#     filtered_state_estimates = kf.filter(data[:i-n_ahead])[0]
#     states, observations = kf.sample(n_ahead, filtered_state_estimates[-1])
#     full_predictions.append(observations[-1][0])

n_ahead = 10

# from sklearn.linear_model import ARDRegression, LinearRegression
X_train = np.transpose(np.array([data[0:780-3],data[1:780-2],data[2:780-1],data[3:780]]))
y_train = np.array(data[n_ahead+3:780+n_ahead])
X_test = np.transpose(np.array([data[780-3:780+52-3],data[780-2:780-2+52],data[780-1:780+52-1],data[780:780+52]]))
import pymc3 as pm
