...
def update(particles, weights,ts,t):
    ...
    for p in range(len(particles)):
        multiplier = scipy.stats.binom.pmf(ts[t],20000,particles[p][1])
...
estimated_states, particles = run_pf1(time_series,N=1000)   
#estimated_states = scaler.inverse_transform(estimated_states)

plot_particles = np.array(plot_particles)[:,:,1]

for xe, ye in zip(range(len(plot_weights)), plot_weights):
    plt.scatter([xe] * len(ye), ye)
    
plt.show()

for xe, ye in zip(range(len(plot_particles)), plot_particles):
    plt.scatter([xe] * len(ye), ye)

plt.show()
plt.plot(20000*estimated_states)
