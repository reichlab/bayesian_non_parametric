...
def update(particles, weights,ts,t):
    for p in range(len(particles)):
        weights[p] *= scipy.stats.norm.pdf(ts[t],p,10)
...
def run_pf1(time_series,N):
    ...
    for t in range(len(time_series)):
        ...
        weights = multinomal_resample(weights)
...
scaler = preprocessing.StandardScaler().fit(time_series)
