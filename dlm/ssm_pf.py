...
def create_uniform_particles( N):
    particles = np.random.multivariate_normal([1400,1400],[[1,0],[0,1]],N)
    #susceptible
...
def predict(particles,t):
    ...
    new_particles = []
    for p in particles:
        new_particles.append(np.matmul(np.array([[np.cos(2*np.pi/6),np.sin(2*np.pi/6)],[-np.sin(2*np.pi/6),np.cos(2*np.pi/6)]]),p.reshape((2,1))) )
    new_particles = np.array(new_particles).reshape((N,2))
    new_particles += np.random.multivariate_normal([0,0],.0001*np.eye(2),size=N)
...
def update(particles, weights,ts,t):
    ...
    for p in range(len(particles)):
        a, b = (0 - particles[p][0]) / 100, (10000000000 - particles[p][0]) / 100

        multiplier = scipy.stats.truncnorm.pdf(ts[t],a,b,loc=particles[p][0],scale=100)
        print (multiplier,ts[t],particles[p][0])
...
time_series = [1432,2988,3960,4019,2380,1801,3396,3816,3646 ,2922]
