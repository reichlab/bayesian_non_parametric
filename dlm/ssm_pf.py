...
def mh_sampler(data, samples=4, mu_init=.5, proposal_width=.5, plot=False, mu_prior_mu=0, mu_prior_sd=1.):
    mu_current = mu_init
...
def create_uniform_particles( N):
    particles = np.random.dirichlet([1000,10,10],N)
    #susceptible
...
def predict(particles,t):
    N = len(particles)
    beta = beta_global# *(1+.01*np.cos(2*np.pi*t/26))
...
def update(particles, weights,ts,t):
    ...
    for p in range(len(particles)):
        print (2000*particles[p][1])
        multiplier = scipy.stats.poisson.pmf(ts[t],2000*particles[p][1])
        if  math.isnan(multiplier) == True:
            multiplier = 0
        weights[p] *= multiplier
...
def run_pf1(time_series,N):
    ...
    plot_particles.append(particles.tolist())
    ...
    for t in range(len(time_series)):
        ...
        plot_weights.append(weights.tolist())
...
n_ahead = 10#int(sys.argv[1])
last_training_point = 100#int(sys.argv[2])

time_series = time_series[:last_training_point-n_ahead]

estimated_states, particles,weights = run_pf1(time_series,N=5000)   

full_predictions = []

for i in range(last_training_point,last_training_point+52):
    predictions = np.zeros(n_ahead+1)
    num_sim = 1000
    for sim in range(1000):
        pred_states = [np.array(estimated_states[-1]).reshape((1,-1))]
        for i in range(n_ahead):
            pred_states.append(predict(pred_states[i],i))
        predictions += 20000*np.array(pred_states)[:,0,1]
    particles = predict(particles,i)
        # incorporate measurements
        
    weights = update(particles, weights,time_series, i)
    plot_weights.append(weights.tolist())
    indexes = stratified_resample(weights)
    particles,weights = resample_from_index(particles, weights, indexes)
    estimated_states, var = estimate(particles, weights)
    estimated_states = np.array(estimated_states).reshape((1,-1))
    full_predictions.append(predictions[-1]/num_sim)
        
        
    
myList = ','.join(map(str, full_predictions))
print (myList)  
plt.plot(range(52),full_predictions)

plt.plot(range(52),time_series[780:780+52])
