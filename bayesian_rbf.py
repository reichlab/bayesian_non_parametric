...
plt.plot(range(len(data__full)),data__full)
plt.show()
SS = StandardScaler().fit(data__full)

X_train,y_train,X_test,y_test = ts_utils.create_train_test(SS.transform(data__full),2,781)
...
def basisfunc( c, d):
        print (c.shape.eval())
        print (c.repeat(len(centers),axis=1).shape.eval())
        print (d.shape)
        ret_val = T.exp(- (c.repeat(len(centers),axis=1)\
            -d.reshape((-1,50,2)))**2)
        print (ret_val.shape.eval())
        return ret_val

hidden_size = 2

kmeans = KMeans(n_clusters=50, random_state=0).fit(X_train.reshape((len(X_train),2)))
centers = kmeans.cluster_centers_.reshape((-1,1,X_train.shape[2]))
