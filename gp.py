...
y_test = np.array(data__full[781:],dtype=np.float32).reshape((-1,1))
...
import sys
import pymc3 as pm
# print (mean_squared_error(m.predict(X_test)[0],y_test))
...
K_noise = K + noise * tt.eye(len(X_train))

# Add very slight perturbation to the covariance matrix diagonal to improve numerical stability
K_stable = K + 1e-12 * tt.eye(len(X_train))

# Observed data
f = y_train#np.random.multivariate_normal(mean=np.zeros(n), cov=K_noise.eval())
...
with pm.Model() as model:
    ...
    f_sample = pm.Flat('f_sample', shape=(len(X_train), ))

    # Likelihood
    y = pm.MvNormal('y', observed=f, mu=f_sample, cov=noise * tt.eye(len(X_train)), shape=len(X_train))
