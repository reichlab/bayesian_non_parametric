...
for i in range(1,len(data__)):
    #train_data6.append(data__[i-6])
    #train_data5.append(data__[i-5])
    #train_data4.append(data__[i-4])
    #train_data3.append(data__[i-3])
    #train_data2.append(data__[i-2])
    train_data1.append(data__[i-1])
    train_target.append(data__[i])



X_train = np.array(train_data1,dtype=np.float32).reshape((-1,1))
y_train =  np.array(train_target,dtype=np.float32).reshape((-1,1))


X_test = data__full[780:len(data__full)-1]
...
#x_lidar6 = shared(np.array(train_data6,dtype=np.float32).reshape((-1,1)), broadcastable=(False, True))
#x_lidar5 = shared(np.array(train_data5,dtype=np.float32).reshape((-1,1)), broadcastable=(False, True))
#x_lidar4 = shared(np.array(train_data4,dtype=np.float32).reshape((-1,1)), broadcastable=(False, True))
#x_lidar3 = shared(np.array(train_data3,dtype=np.float32).reshape((-1,1)), broadcastable=(False, True))
#x_lidar2 = shared(np.array(train_data2,dtype=np.float32).reshape((-1,1)), broadcastable=(False, True))
x_lidar1 = shared(X_train, broadcastable=(False, True))

with pm.Model() as model:
    alpha = pm.Normal('alpha', 0., 100., shape=K)
    beta_1 = pm.Normal('beta1', 0., 100., shape=K)
   # beta_2 = pm.Normal('beta2', 0., 100., shape=K)
   # beta_3 = pm.Normal('beta3', 0., 100., shape=K)
   # beta_4 = pm.Normal('beta4', 0., 100., shape=K)
   # beta_5 = pm.Normal('beta5', 0., 100., shape=K)
    #beta_6 = pm.Normal('beta6', 0., 100., shape=K)
    v = norm_cdf(alpha + beta_1 * x_lidar1 )#+ beta_2*x_lidar2 + \
   # beta_3*x_lidar3+beta_4*x_lidar4 +beta_5*x_lidar5)#+beta_6*x_lidar6 )
...
with model:
    ...
    gamma_1 = pm.Normal('gamma1', 0., 100., shape=hidden_size)
    gamma_2 = pm.Normal('gamma2', 0., 100., shape=K)

    delta_1 = pm.Normal('delta1', 0., 100., shape=(1,hidden_size))
    delta_2 = pm.Normal('delta2', 0., 100., shape=(hidden_size,K))
    #tmp_ = tt.stack([x_lidar1,x_lidar2,x_lidar3,x_lidar4,x_lidar5],axis=2).reshape((-1,5))
    tmp_ = tt.stack([x_lidar1]).reshape((-1,1))
    h =  tt.nnet.relu(gamma_1+ tt.dot(tmp_,delta_1))
    mu = pm.Deterministic('mu', gamma_2+tt.dot(h,delta_2))
...
# x_test5 = []
# x_test6 = []
for i in range(780,987):
    #x_test2.append(data__full[i-1])
    x_test1.append(data__full[i])
   # x_test3.append(data__full[i-2])
   # x_test4.append(data__full[i-3])
   # x_test5.append(data__full[i-4])
   # x_test6.append(data__full[i-5])
x_lidar1.set_value(np.array(X_test,dtype=np.float32).reshape((-1,1)))
#x_lidar2.set_value(np.array(x_test2,dtype=np.float32).reshape((-1,1)))
#x_lidar3.set_value(np.array(x_test3,dtype=np.float32).reshape((-1,1)))
#x_lidar4.set_value(np.array(x_test4,dtype=np.float32).reshape((-1,1)))
#x_lidar5.set_value(np.array(x_test5,dtype=np.float32).reshape((-1,1)))
#x_lidar6.set_value(np.array(x_test6,dtype=np.float32).reshape((-1,1)))
...
for elm in range(50):
    ...
    dist1 = np.histogram(np.transpose(pp_trace['obs'])[elm],bins=range(-50,100),density=True)
    probs = dist1[0]
    vals = dist1[1]
...
print (mean_squared_error(pp_trace['obs'].mean(axis=0),y_test))
#print (np.prod(pp_trace['obs'].mean(axis=0)))
#print (pp_trace['obs'].shape)#.mean(axis=0)
#plt.errorbar(x=np.array(data__full[100:150]), y=np.asarray(pp_trace['obs']).mean(axis=0), yerr=np.asarray(pp_trace['obs']).std(axis=0), linestyle='', marker='o')
#plt.plot(np.array(data__full[100:150]), data__full[101:151], 'o')
#plt.ylim(-5, 100)
#plt.xlim(-5,100)
