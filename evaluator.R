library(Metrics)
source("/Users/gcgibson/Desktop/bayesian_non_parametric/kcde/kcde.R")
source("/Users/gcgibson/Desktop/bayesian_non_parametric/dlm/dengue_dlm.R")
source("/Users/gcgibson/Desktop/bayesian_non_parametric/dengue_sj_experiments/sarima/sarima-estimation.R")
source("/Users/gcgibson/Desktop/bayesian_non_parametric/dp/dp.R")
#source("/Users/gcgibson/Desktop/bayesian_non_parametric/dlm/dssm.R")
...
max_lag_val <- 4
last_training_points <- 52*seq(3,15)
n_aheads <- 1:10

cnames <- c("data", "n_ahead", "last_training_point", "dpForecast", "sarimaForecast", "dlmForecast")
results <- matrix(NA,
...
for (n_ahead in n_aheads){
 for (last_training_point in last_training_points){
   running_idx <- idx:(idx+51)
   ...
   #ssmForecasts <- train_and_predict_dssm(sanJuanDengueData,n_ahead,last_training_point)
...
cnames <- c( "n_ahead", "last_training_point", "dpForecast", "sarimaForecast", "dlmForecast")


mse_results <- matrix(NA,
                  nrow = length(n_aheads)*length(last_training_points)+1,
                  ncol = length(cnames))
colnames(mse_results) <-cnames
